...
import in.cdac.university.studentWelfare.entity.GmstConfigTemplateComponentDtl;
import in.cdac.university.studentWelfare.entity.GmstConfigTemplateComponentDtlPK;
...
public interface TemplateComponentDetailRepository
	...
	@Query(value = "update university.gmst_config_template_component_dtl a set "
			...
			+ " where a.unum_temple_comp_id = b.unum_temple_comp_id and unum_isvalid > 2), udt_eff_to = current_timestamp where"
			+ " unum_temple_comp_id = :unumTemplCompId and unum_isvalid = 1", nativeQuery = true)
	...
	@Query(value = "update university.gmst_config_template_component_dtl a set "
			+ " unum_isvalid = (select coalesce(max(unum_isvalid), 2) + 1 from university.gmst_config_template_component_dtl b "
			+ " where a.unum_temple_comp_id = b.unum_temple_comp_id and unum_isvalid > 2), udt_eff_to = current_timestamp where"
			+ " unum_temple_comp_id in (:unumTemplCompId) and unum_isvalid = 1", nativeQuery = true)

...
import in.cdac.university.studentWelfare.bean.*;
import in.cdac.university.studentWelfare.entity.*;
...
//    private ApplicantRepository applicantRepository;
    ...
    public ServiceResponse getTemplate(Long masterTemplateId, Long notificationId, Long notificationDetailId) throws Exception {
        ...
        NotificationBean notificationBean = restService.getNotificationById(notificationId);
...
//        }
    ...
    private void processTemplateItems(Integer schemeId, Integer universityId, String facultyName, String purpose, Map<Long, String> itemMap,
        ...
        List<GmstConfigTemplateItemMst> itemsByTemplateId = templateItemRepository.findItemsByTemplateId(universityId, templateIds)
                .stream()
                .filter(item -> switch (schemeId) {
                    case 1 -> item.getUnumSbphuleFlag() != null && item.getUnumSbphuleFlag() == 1;
                    case 2 -> item.getUnumDhanvantariFlag() != null && item.getUnumDhanvantariFlag() == 1;
                    case 3 -> item.getUnumEarnwhilelearnFlag() != null && item.getUnumEarnwhilelearnFlag() == 1;
                    case 4 -> item.getUnumBahishalFlag() != null && item.getUnumBahishalFlag() == 1;
                    case 5 -> item.getUnumSanjeevniFlag() != null && item.getUnumSanjeevniFlag() == 1;
        ...
        Map<Long, Integer> itemControlMap = itemsByTemplateId.stream()
                .collect(Collectors.toMap(GmstConfigTemplateItemMst::getUnumTempleItemId, item -> Optional.ofNullable(item.getUnumUiControlId()).orElse(0)));
        ...
        for (TemplateBean templateBean : templateBeans) {
            ...
            for (GmstConfigTemplateDtl configTemplateDtl : templateDetailByTemplateId) {
                if (configTemplateDtl.getUnumTempleHeadId() == 27 || !headerIdsAdded.containsKey(configTemplateDtl.getUnumTempleHeadId())) {
                    ...
                    if (configTemplateDtl.getUstrHeaderStyle() != null)
                        templateHeaderBean.setUstrHeaderStyle(configTemplateDtl.getUstrHeaderStyle());
            ...
            for (TemplateHeaderBean templateHeaderBean : headerBeans) {
                ...
                List<TemplateComponentBean> templateComponentBeans = componentsByTemplateId.stream()
                        ...
                        .map(gmstConfigTemplateComponentMst -> {
                            ...
                            if (templateDtl.getUstrCompStyle() != null)
                                templateComponentBean.setUstrCompStyle(templateDtl.getUstrCompStyle());
                ...
                for (TemplateComponentBean templateComponentBean : templateComponentBeans) {
                    ...
                    List<TemplateItemBean> templateItemBeans = itemsByTemplateId.stream()
                            ...
                            .map(gmstConfigTemplateItemMst -> {
                                ...
                                if (templateDtl.getUstrItemStyle() != null)
                                    templateItemBean.setUstrItemStyle(templateDtl.getUstrItemStyle());
        ...
        List<TemplateItemBean> childItems = itemsByTemplateId.stream()
                ...
                .map(gmstConfigTemplateItemMst -> {
                    ...
                    if (gmstConfigTemplateDtl != null) {
                        ...
                        if (gmstConfigTemplateDtl.getUstrItemStyle() != null)
                            itemBean.setUstrItemStyle(gmstConfigTemplateDtl.getUstrItemStyle());
                    }
                    return itemBean;
                })
                .sorted(Comparator.comparing(TemplateItemBean::getUnumItemDisplayOrder, Comparator.nullsLast(Comparator.naturalOrder())))
